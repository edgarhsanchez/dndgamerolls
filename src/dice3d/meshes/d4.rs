...
pub fn create_d4() -> (Mesh, Collider, Vec<(Vec3, u32)>) {
    ...
    let positions: Vec<[f32; 3]> = vec![
        ...
        v0.into(), v2.into(), v3.into(),
        // Left-back face (v0, v3, v1)
        v0.into(), v3.into(), v1.into(),
    ];
    
    let normals: Vec<[f32; 3]> = vec![
        mesh_n_base.into(), mesh_n_base.into(), mesh_n_base.into(),
        mesh_n_front.into(), mesh_n_front.into(), mesh_n_front.into(),
        mesh_n_right.into(), mesh_n_right.into(), mesh_n_right.into(),
        mesh_n_left.into(), mesh_n_left.into(), mesh_n_left.into(),
    ];
...
pub fn get_d4_number_positions() -> Vec<(Vec3, Quat, u32)> {
    ...
    let calc_rotation = |face_normal: Vec3, _face_center: Vec3, number_pos: Vec3, target_vertex: Vec3| -> Quat {
    ...
    let pos_a1 = face_a_center + (v1 - face_a_center).normalize() * label_offset_from_center + n_a * surface_offset;
    let pos_a2 = face_a_center + (v2 - face_a_center).normalize() * label_offset_from_center + n_a * surface_offset;
    let pos_a3 = face_a_center + (v3 - face_a_center).normalize() * label_offset_from_center + n_a * surface_offset;
    ...
    let pos_b0 = face_b_center + (v0 - face_b_center).normalize() * label_offset_from_center + n_b * surface_offset;
    let pos_b1 = face_b_center + (v1 - face_b_center).normalize() * label_offset_from_center + n_b * surface_offset;
    let pos_b2 = face_b_center + (v2 - face_b_center).normalize() * label_offset_from_center + n_b * surface_offset;
    ...
    let pos_c0 = face_c_center + (v0 - face_c_center).normalize() * label_offset_from_center + n_c * surface_offset;
    let pos_c2 = face_c_center + (v2 - face_c_center).normalize() * label_offset_from_center + n_c * surface_offset;
    let pos_c3 = face_c_center + (v3 - face_c_center).normalize() * label_offset_from_center + n_c * surface_offset;
    ...
    let pos_d0 = face_d_center + (v0 - face_d_center).normalize() * label_offset_from_center + n_d * surface_offset;
    let pos_d3 = face_d_center + (v3 - face_d_center).normalize() * label_offset_from_center + n_d * surface_offset;
    let pos_d1 = face_d_center + (v1 - face_d_center).normalize() * label_offset_from_center + n_d * surface_offset;
