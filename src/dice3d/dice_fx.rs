...
use crate::dice3d::systems::dice_fx::{
    ...
    tick_dice_fx_material_time, expire_custom_dice_fx, sync_dice_fx_plume_transforms,
    billboard_dice_fx_plumes_to_camera,
};
...
impl Plugin for DiceFxPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(bevy::pbr::MaterialPlugin::<DiceSurfaceFxMaterial>::default())
            ...
            .add_systems(
                Update,
                sync_dice_fx_plume_transforms.after(sync_dice_fx_visuals),
            )
            .add_systems(
                Update,
                billboard_dice_fx_plumes_to_camera.after(sync_dice_fx_plume_transforms),
            )
            .add_systems(
                Update,
